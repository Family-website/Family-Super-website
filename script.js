...
// ==========================================
// 🖨️ 100% FIXED PRINT & PREMIUM PDF REPORTS
// ==========================================

// FIXED: Print Function using Hidden iframe (Works 100% on Mobile/Desktop)
function printReport() { 
    ...
    exps.sort((a,b)=>new Date(b.date)-new Date(a.date)).forEach(e => {
        trs += `<tr><td style="padding:8px; border:1px solid #ddd;">${e.date}</td><td style="padding:8px; border:1px solid #ddd;">${e.category}</td><td style="padding:8px; border:1px solid #ddd;">${e.description}</td><td style="padding:8px; border:1px solid #ddd;">${e.member}</td><td style="padding:8px; border:1px solid #ddd;">Rs ${e.amount}</td></tr>`;
    ...
    doc.open();
    doc.write('<html><head><title>Print Report</title></head><body>' + html + '</body></html>');
    doc.close();
...
// PREMIUM PDF DESIGN & SMART DOWNLOAD
async function shareReport() {
    ...
    [...dataToExport].sort((a, b) => new Date(b.date) - new Date(a.date)).forEach(exp => { const p = exp.date.split('-'); tableRows.push([`${p[2]}/${p[1]}`, exp.member || '-', exp.category || 'Other', exp.description, `Rs ${exp.amount}`]); });
