...
// ==========================================
// ☁️ CLOUD DATA & GLOBALS (Phase 21 Recharges Added)
// ==========================================
...
// ==========================================
// 📱 PHASE 21: RECHARGE TRACKER LOGIC
// ==========================================
function addRecharge() {
    ...
    let startDate = new Date(dDate);
    startDate.setDate(startDate.getDate() + days);
    let expDate = startDate.toISOString().split('T')[0];

    rechargeRecords.push({ name: name, amount: amt, days: days, date: dDate, expiry: expDate });
    
    // Add to Hisaab automatically
    familyExpenses.push({ member: "Aditya", category: "Bills", description: `📱 Recharge: ${name}`, amount: amt, date: dDate, receipt: "", gps: null });

    saveToCloud(); updateRechargeUI(); updateHisabUI();
    document.getElementById('rech-name').value = ''; document.getElementById('rech-amt').value = ''; document.getElementById('rech-days').value = '';
...
function updateRechargeUI() {
    ...
    rechargeRecords.sort((a, b) => new Date(b.date) - new Date(a.date)).forEach((rech, index) => {
        let expD = new Date(rech.expiry);
        let diffTime = expD - today;
...
function openProfile() {
    try {
        ...
        // 🛡️ BUG FIX: ONLY RENDER CHARTS WHEN MODAL IS OPEN (Prevents Crash)
        setTimeout(() => {
...
// ==========================================
// ⚡ PHASE 22: SMART BOTTOM SHEET UI
// ==========================================
function openAIBottomSheet() {
    Swal.fire({
        title: '🤖 AI Finance Assistant',
        html: `<div style="display:flex; flex-direction:column; gap:10px;">
                <button onclick="Swal.close(); askFinanceAI();" style="background:#6366f1; color:white; border:none; padding:12px; border-radius:12px; font-weight:bold;">📊 Get AI Spending Report</button>
                <button onclick="Swal.close(); startVoice();" style="background:#8b5cf6; color:white; border:none; padding:12px; border-radius:12px; font-weight:bold;">🎤 Add Kharcha by Voice</button>
                <p style="font-size:11px; color:#64748b; margin-top:5px;">Real-time AI calculations based on your history.</p>
               </div>`,
...
function renderCalendar(exps, fm) {
    ...
    for(let i=1; i<=dim; i++) {
        ...
        // 📊 PHASE 22: HEATMAP UPGRADE
        if(dt[i]) { 
            el.style.color = 'white'; 
            if(dt[i] > 2000) el.style.background = '#7f1d1d'; // Dark Red (Very High)
            else if(dt[i] > 1000) el.style.background = '#ef4444'; // Red
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if(descInp) {
        descInp.addEventListener('input', function(e) {
            ...
            else if(val.includes('petrol') || val.includes('diesel') || val.includes('bike')) cat.value = 'Petrol';
            else if(val.includes('sabji') || val.includes('ration') || val.includes('tel')) cat.value = 'Ration';
            else if(val.includes('recharge') || val.includes('bill') || val.includes('wifi')) cat.value = 'Bills';
            else if(val.includes('kapde') || val.includes('shoes') || val.includes('shopping')) cat.value = 'Shopping';
        });
    }
});
