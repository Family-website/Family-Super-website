...
// ==========================================
let localData = JSON.parse(localStorage.getItem('appDataCache') || '{}');
...
// Data ab hamesha local memory se load hoga taki refresh karne par 0 na ho
    ...
    try {
        db.collection('familyData').doc(uid).onSnapshot((doc) => {
            if (doc.exists) {
                ...
                // Save locally so it doesn't reset to 0
...
function addExpense() {
    try {
        ...
        const preview = document.getElementById('receipt-preview'); if(preview) preview.style.display = 'none'; if(receiptInput) receiptInput.value = ""; 
        
        saveToCloud(); 
        updateHisabUI(); // Direct update, no timeout lag
...
function startVoice() { 
    ...
    try {
        ...
        recognition.start(); 
    } catch(e) { document.getElementById('mic-btn').innerText = "🎤"; console.error(e); Swal.fire('Error', 'Mic start nahi hua. Permission check karein.', 'error'); }
}
