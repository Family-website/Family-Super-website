...
function updateSoundUI() { const btn = document.getElementById('sound-toggle-btn'); if(btn) { btn.innerHTML = isSoundEnabled ? '<i class="fa-solid fa-volume-high"></i> Sound: ON' : '<i class="fa-solid fa-volume-xmark"></i> Sound: OFF'; btn.style.color = isSoundEnabled ? '#10b981' : '#64748b'; btn.style.borderColor = isSoundEnabled ? '#10b981' : '#64748b'; } }
...
// ==========================================
function showSyncSuccess() { const syncEl = document.getElementById('sync-status'); if(syncEl) { syncEl.innerHTML = '<i class="fa-solid fa-cloud-check"></i> Synced Just Now'; syncEl.style.color = "#10b981"; setTimeout(() => { syncEl.style.color = "#94a3b8"; syncEl.innerHTML = '<i class="fa-solid fa-cloud"></i> Cloud Active'; }, 3000); } }
...
async function saveToCloud() { 
    ...
    try {
        ...
        const s = document.getElementById('sync-status'); if(s) { s.innerHTML = '<i class="fa-solid fa-cloud-check"></i> Synced'; s.style.color = "#10b981"; setTimeout(() => { s.style.color = "#94a3b8"; s.innerHTML = '<i class="fa-solid fa-cloud"></i> Synced'; }, 2000); }
    } catch(e){}
}
...
document.addEventListener('touchend', e => {
    ...
    if(pEndY > pStartY + 150 && window.scrollY === 0 && currentUser) {
        playSound('click'); pRef.style.display = 'block'; pRef.innerHTML = '<i class="fa-solid fa-rotate fa-spin"></i> Syncing...';
        setTimeout(() => { if(!isOffline) loadCloudData(currentUser.uid); pRef.innerHTML = '<i class="fa-solid fa-check"></i> Synced!'; playSound('success'); setTimeout(() => pRef.style.display = 'none', 1000); }, 1000);
    }
});
...
function updatePrediction(tot) { 
    ...
    el.innerText = `₹${Math.round(p)} ${appLang==='English'?'Expected':'Umeed'}`; 
...
function captureLocation() { let statusEl = document.getElementById('loc-status'); statusEl.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Fetching...'; if (navigator.geolocation) { navigator.geolocation.getCurrentPosition((pos) => { currentGPSLocation = `https://www.google.com/maps/search/?api=1&query=$${pos.coords.latitude},${pos.coords.longitude}`; statusEl.innerHTML = '<i class="fa-solid fa-check"></i> Captured!'; statusEl.style.color = "#10b981"; playSound('click'); }, () => { statusEl.innerHTML = '<i class="fa-solid fa-xmark"></i> Failed'; statusEl.style.color = "#ef4444"; Swal.fire('Error', 'Location on karein!', 'error'); }); } else { statusEl.innerHTML = '<i class="fa-solid fa-xmark"></i> Not Supported'; } playSound('click'); }
    ...
    if(filteredExpenses.length === 0) {
        ...
        filteredExpenses.forEach((item) => {
            if(isDateSorted && item.date !== currentDateHeader) {
                ...
                const dateHeader = document.createElement('div'); dateHeader.className = 'date-header'; dateHeader.style.fontWeight = 'bold'; dateHeader.style.color = 'var(--ink-blue)'; dateHeader.style.margin = '10px 0 5px 0'; dateHeader.innerHTML = `<i class="fa-regular fa-calendar"></i> ${showDate}`; fragment.appendChild(dateHeader);
            }
...
function addExpense() {
    try {
        ...
        if (typeof editExpenseIndex !== 'undefined' && editExpenseIndex !== -1) {
            ...
            if(btnAdd) btnAdd.innerHTML = '<i class="fa-solid fa-plus"></i> Add Kharcha';
        ...
        if(statusEl) { statusEl.innerHTML = '<i class="fa-solid fa-location-dot"></i> Not Saved'; statusEl.style.color = "var(--text-muted)"; }
...
async function sendToRealAI() {
    ...
    chatBox.innerHTML += `<div id="${loadingId}" style="margin:5px 0; text-align:left;"><span style="background:var(--paper-bg); color:var(--text-main); border:1px solid #cbd5e1; padding:8px 12px; border-radius:16px 16px 16px 0; display:inline-block;"><i class="fa-solid fa-spinner fa-spin"></i> AI is thinking...</span></div>`;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            chatBox.innerHTML += `<div style="margin:5px 0; text-align:left;"><span style="background:#fee2e2; color:#991b1b; padding:8px 12px; border-radius:16px 16px 16px 0; display:inline-block; font-size:11px;"><i class="fa-solid fa-triangle-exclamation"></i> <b>API Error:</b> ${errMsg}</span></div>`;
            chatBox.scrollTop = chatBox.scrollHeight;
            return;
        }
        ...
        if (data.candidates && data.candidates[0].content) {
            ...
            chatBox.innerHTML += `<div style="margin:5px 0; text-align:left;"><span style="background:#fee2e2; color:#991b1b; padding:8px 12px; border-radius:16px 16px 16px 0; display:inline-block;"><i class="fa-solid fa-triangle-exclamation"></i> AI ne koi jawab nahi diya.</span></div>`;
        }
        
        chatBox.scrollTop = chatBox.scrollHeight; playSound('success');
    } catch(e) {
        if(document.getElementById(loadingId)) document.getElementById(loadingId).remove();
        chatBox.innerHTML += `<div style="margin:5px 0; text-align:left;"><span style="background:#fee2e2; color:#991b1b; padding:8px 12px; border-radius:16px 16px 16px 0; display:inline-block; font-size:11px;"><i class="fa-solid fa-wifi"></i> Internet ya Network Error! Check console.</span></div>`;
...
function startVoice() { 
    ...
    try {
        ...
        recognition.lang = 'hi-IN'; const btn = document.getElementById('mic-btn'); btn.innerHTML = '<i class="fa-solid fa-stop"></i>'; 
        ...
        recognition.onerror = (e) => { btn.innerHTML = '<i class="fa-solid fa-microphone"></i>'; console.error(e); if(e.error === 'not-allowed') { Swal.fire('Permission Denied', 'Browser me Microphone ki permission on karein!', 'error'); } else { Swal.fire('Error', 'Awaz clear nahi aayi!', 'error'); } }; 
        recognition.onend = () => { btn.innerHTML = '<i class="fa-solid fa-microphone"></i>'; }; recognition.start(); playSound('click');
    } catch(e) { document.getElementById('mic-btn').innerHTML = '<i class="fa-solid fa-microphone"></i>'; console.error(e); Swal.fire('Error', 'Mic start nahi hua. ' + e.message, 'error'); }
}
...
function openAIBottomSheet() {
    Swal.fire({
        ...
        html: `<div style="display:flex; flex-direction:column; gap:10px;">
                <button onclick="Swal.close(); askFinanceAI();" class="expense-btn" style="background:#6366f1; color:white; border:none; padding:12px; border-radius:12px; font-weight:bold;"><i class="fa-solid fa-chart-pie"></i> Get AI Spending Report</button>
                <button onclick="Swal.close(); startVoice();" class="expense-btn" style="background:#8b5cf6; color:white; border:none; padding:12px; border-radius:12px; font-weight:bold;"><i class="fa-solid fa-microphone"></i> Add Kharcha by Voice</button>
                <hr style="border: 0.5px dashed #cbd5e1; margin: 5px 0;">
                <p style="font-size:12px; font-weight:bold; color:var(--ink-blue); text-align:left; margin:0;"><i class="fa-solid fa-comment-dots"></i> Chat with AI</p>
                ...
                <div style="display:flex; gap:8px; align-items:center;">
                    ...
                    <button onclick="sendToRealAI()" style="background:#10b981; color:white; border:none; padding:0 20px; border-radius:16px; font-weight:bold; cursor:pointer; height:50px; display:flex; align-items:center; justify-content:center;"><i class="fa-solid fa-paper-plane"></i></button>
                </div>
               </div>`,
