...
window.addEventListener('load', () => {
    setTimeout(() => {
        ...
        let obDone = localStorage.getItem('onboarding_done');
        if(!obDone) { document.getElementById('onboarding-screen').style.display = 'flex'; } 
...
auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        if(obDone) {
            ...
            if(document.getElementById('pin-screen')) { document.getElementById('pin-screen').style.display = 'flex'; if(!savedPin) { document.getElementById('pin-msg').innerText = "Security ke liye naya 4-digit PIN banayein"; document.getElementById('btn-setup-pin').style.display = 'block'; } } 
...
function loginWithEmail() { const email = document.getElementById('email-input').value.trim(); const password = document.getElementById('password-input').value.trim(); if (!email || password.length < 6) return Swal.fire('Oops!', 'Sahi email aur password daalein.', 'warning'); auth.signInWithEmailAndPassword(email, password).catch(() => Swal.fire('Error', 'Email ya password galat hai!', 'error')); }
function registerWithEmail() { const email = document.getElementById('email-input').value.trim(); const password = document.getElementById('password-input').value.trim(); if (!email || password.length < 6) return Swal.fire('Oops!', 'Details daalein.', 'warning'); auth.createUserWithEmailAndPassword(email, password).then(() => Swal.fire('Mubarak ho!', 'Account ban gaya!', 'success')).catch(e => Swal.fire('Error', e.message, 'error')); }
function logout() { Swal.fire({ title: 'Logout?', icon: 'warning', showCancelButton: true }).then((result) => { if (result.isConfirmed) auth.signOut(); }); }

function setupPin() { let pin = document.getElementById('pin-input').value; if(pin.length === 4 && !isNaN(pin)) { localStorage.setItem('app_pin', pin); Swal.fire('Secured! 🔒', 'PIN set ho gaya!', 'success'); document.getElementById('pin-screen').style.display = 'none'; document.getElementById('main-app').style.display = 'block'; } else Swal.fire('Error', '4 number ka PIN daalein!', 'error'); }
function verifyPin() { let pin = document.getElementById('pin-input').value; let savedPin = localStorage.getItem('app_pin'); if(!savedPin) return Swal.fire('Wait', 'Setup PIN pehle karein', 'info'); if(pin === savedPin) { document.getElementById('pin-screen').style.display = 'none'; document.getElementById('main-app').style.display = 'block'; playSound('success'); } else { Swal.fire('Galat PIN ❌', '', 'error'); document.getElementById('pin-input').value = ""; } }
...
async function syncOldLocalData() { const localExp = JSON.parse(localStorage.getItem('familyExpenses')); if (localExp && localExp.length > 0 && familyExpenses.length === 0) { familyExpenses = localExp; await saveToCloud(); localStorage.removeItem('familyExpenses'); } }

function checkSmartReminders() {
    let td = new Date().getDate(); let rs = sessionStorage.getItem('reminderShownToday');
    if(!rs) {
        let la = activeLoans.filter(l => l.monthsPaid < l.time && Math.abs(l.dueDate - td) <= 2).map(l => l.name);
        if(la.length > 0) Swal.fire('🔔 EMI Alert!', `Mahaul tight hai! "${la.join(', ')}" ki EMI aane wali hai.`, 'warning');
        else if(td >= 1 && td <= 5) Swal.fire('🔔 Mahine ki shuruat!', 'Kiraya/bills baaki hain toh clear kar lo!', 'info');
        let ls = rationItems.filter(i => i.lowStock).length; if(ls >= 3) setTimeout(() => Swal.fire('🛒 Ration Khatam!', `${ls} ration items low stock par hain.`, 'warning'), 3000);
...
// ==========================================
// 🛡️ USER PROFILE, XP & BUG FIX
// ==========================================
function updateProfileName() { 
    const currentName = customDisplayName || (currentUser && currentUser.email ? currentUser.email.split("@")[0] : "User"); 
    Swal.fire({ title: 'Apna Naam Likhein', input: 'text', inputValue: currentName, showCancelButton: true, confirmButtonText: 'Save', confirmButtonColor: '#2563eb' }).then((result) => { if (result.isConfirmed) { customDisplayName = result.value.trim(); saveToCloud(); updateGreetingName(); Swal.fire('Saved!', '', 'success'); } }); 
}
...
function updateGreetingName() { 
    ...
    document.getElementById('profile-name').innerText = NameFormatted; document.getElementById('user-avatar').innerText = finalName.charAt(0).toUpperCase(); document.getElementById('profile-avatar-large').innerText = finalName.charAt(0).toUpperCase(); 
}
...
function openProfile() {
    try {
        ...
        if(lvlBadge) { let level = Math.floor((parseInt(userXP) || 0) / 100) + 1; let title = level < 3 ? "Beginner 🥉" : level < 6 ? "Pro Saver 🥈" : "Finance Ninja 🥇"; lvlBadge.innerText = `Level ${level} | ${title} (XP: ${userXP})`; }

        let totalExpAllTime = familyExpenses.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0); 
        let expEl = document.getElementById('profile-total-expense'); if(expEl) expEl.innerText = `₹${totalExpAllTime}`;
        ...
        let dudhEl = document.getElementById('profile-total-dudh'); if(dudhEl) dudhEl.innerText = `₹${Math.round(totalDudhAllTime)}`;
        ...
        let nwEl = document.getElementById('profile-net-worth'); if(nwEl) { if(netWorth >= 0) { nwEl.style.color = '#10b981'; nwEl.innerText = `₹${Math.round(netWorth)} 📈`; } else { nwEl.style.color = '#ef4444'; nwEl.innerText = `₹${Math.round(netWorth)} 📉`; } }
        ...
        if(mIncome > 0) { let savePercent = ((mIncome - monthExpenses) / mIncome) * 100; if(savePercent >= 20) score = 95; else if(savePercent >= 10) score = 75; else if(savePercent >= 0) score = 60; else score = 30; }
        ...
        let monthData = familyExpenses.filter(item => item.date && item.date.startsWith(filterMonth)); let memTotals = {}; monthData.forEach(exp => { let m = exp.member || "Unknown"; memTotals[m] = (memTotals[m] || 0) + (parseFloat(exp.amount) || 0); });
        let sortedMembers = Object.keys(memTotals).map(m => ({ name: m, amount: memTotals[m] })).sort((a,b) => b.amount - a.amount);
        let lList = document.getElementById('leaderboard-list'); if(lList) { lList.innerHTML = ''; const medals = ['🥇', '🥈', '🥉']; sortedMembers.forEach((mem, idx) => { let medal = idx < 3 ? medals[idx] : '😎'; const li = document.createElement('li'); li.style.background = 'transparent'; li.style.borderBottom = '1px dashed #fcd34d'; li.style.padding = '8px 0'; li.style.display = 'flex'; li.style.justifyContent = 'space-between'; li.innerHTML = `<span style="font-weight:bold; color:#b45309;">${medal} ${mem.name}</span> <span style="font-weight:900; color:#d97706;">₹${mem.amount}</span>`; lList.appendChild(li); }); }
...
function gainXP(amount) { userXP += amount; saveToCloud(); Swal.mixin({ toast: true, position: 'top-end', showConfirmButton: false, timer: 1500 }).fire({ icon: 'success', title: `+${amount} XP Earned!` }); }
function progressChallenge() { if(challengeDays >= 30) return Swal.fire('Wah!', '30 din poore!', 'success'); challengeDays += 1; gainXP(50); saveToCloud(); updateChallengeUI(); playSound('success'); if(typeof confetti !== 'undefined') confetti({ particleCount: 50, spread: 50, origin: { y: 0.6 } }); }
function updateChallengeUI() { let bar = document.getElementById('challenge-bar'); let text = document.getElementById('challenge-days'); if(bar && text) { bar.style.width = `${(challengeDays / 30) * 100}%`; text.innerText = challengeDays; } }
...
function addToDo() { Swal.fire({ title: 'Naya Task', input: 'text', showCancelButton: true, confirmButtonColor: '#f59e0b' }).then((res) => { if(res.isConfirmed && res.value.trim()) { todoItems.push({ text: res.value.trim(), done: false }); saveToCloud(); updateToDoUI(); playSound('click'); } }); }
...
function updatePrediction(tot) { let d = new Date().getDate(); let dim = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate(); let p = (tot / d) * dim; if(isNaN(p) || !isFinite(p)) p = 0; const el = document.getElementById('predicted-expense'); if(el) { el.innerText = `₹${Math.round(p)} ${appLang==='English'?'Expected':'lagne ki umeed hai'}`; el.style.color = p > budgetLimit ? '#ef4444' : '#3730a3'; } }
...
function renderCalendar(exps, fm) {
    ...
    for(let i=1; i<=dim; i++) {
        ...
        if(dt[i]) { el.style.color = 'white'; el.style.background = dt[i] > 1000 ? '#ef4444' : dt[i] > 300 ? '#f59e0b' : '#10b981'; } else { el.style.background = 'var(--line-color)'; el.style.color = 'var(--text-main)'; }
        cel.appendChild(el);
    }
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if(descInp) {
        descInp.addEventListener('input', function(e) {
...
let searchTimeout;
function debounceSearch() { clearTimeout(searchTimeout); searchTimeout = setTimeout(() => { updateHisabUI(); }, 300); }
...
function renderHistoryWithSkeleton() { const list = document.getElementById('history-list'); if(!list) return; list.innerHTML = `<div class="skeleton-box" style="height:60px; margin-bottom:10px;"></div><div class="skeleton-box" style="height:60px;"></div>`; setTimeout(updateHisabUI, 400); }
...
// ==========================================
// 💾 SECURE BACKUP, PDF & EXCEL REPORTS 
// ==========================================
function backupData() { 
...
// ==========================================
// 🧠 11. REAL AI INSIGHTS (Phase 17 Update)
// ==========================================
function startVoice() { 
...
function askFinanceAI() {
    ...
    Swal.fire({ 
        ...
        icon: 'info', confirmButtonText: 'Thanks AI! 👍', confirmButtonColor: '#6366f1' 
    });
}
