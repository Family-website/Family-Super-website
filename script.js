...
// ==========================================
function showSyncSuccess() { const syncEl = document.getElementById('sync-status'); if(syncEl) { syncEl.innerText = "Synced Just Now"; syncEl.style.color = "#10b981"; setTimeout(() => { syncEl.style.color = "#94a3b8"; syncEl.innerText = "Cloud Active"; }, 3000); } }
...
async function saveToCloud() { 
    ...
    try {
        ...
        const s = document.getElementById('sync-status'); if(s) { s.innerText = "Synced"; s.style.color = "#10b981"; setTimeout(() => s.style.color = "#94a3b8", 2000); }
    } catch(e){}
}
...
// ── FEATURE NEW-1: Cash Flow Forecast ────────────────
function openCashFlowForecast() {
    const fm = todayDateString.slice(0,7);
    const today = new Date();
    ...
    const monthExp = familyExpenses.filter(e=>e.date&&e.date.startsWith(fm)).reduce((s,e)=>s+parseFloat(e.amount),0);
    const dailyAvg = daysPassed > 0 ? monthExp / daysPassed : 0;
    const projectedEnd = Math.round(monthExp + (dailyAvg * daysLeft));
    const projectedSavings = monthlyIncome - projectedEnd;
    const upcomingEMI = activeLoans.reduce((s,l)=>s+(l.emi||0),0);
    const upcomingSubs = activeSubs.reduce((s,sb)=>{
        const due = parseInt(sb.dueDate||'28');
        const todayDate = today.getDate();
        if(due >= todayDate) return s + (parseFloat(sb.amount)||0);
        return s;
    },0);
    Swal.fire({
        title: '',
        html: `<div style="text-align:left;">
            <div style="background:linear-gradient(135deg,#1e293b,#334155);border-radius:20px;padding:18px;margin-bottom:16px;color:white;">
                <p style="font-size:11px;color:#94a3b8;font-weight:800;text-transform:uppercase;margin-bottom:4px;">Cash Flow Forecast</p>
                <p style="font-size:13px;color:#cbd5e1;margin:0;">${fm} | ${daysLeft} din baaki</p>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
                <div style="background:#f0fdf4;border-radius:14px;padding:14px;border:1px solid #86efac;">
                    <p style="font-size:10px;font-weight:800;color:#166534;margin-bottom:4px;">ABHI TAK KHARCHA</p>
                    <p style="font-size:18px;font-weight:900;color:#15803d;margin:0;">₹${monthExp.toLocaleString('en-IN')}</p>
                </div>
                <div style="background:${projectedEnd>monthlyIncome?'#fef2f2':'#eff6ff'};border-radius:14px;padding:14px;border:1px solid ${projectedEnd>monthlyIncome?'#fca5a5':'#93c5fd'};">
                    <p style="font-size:10px;font-weight:800;color:${projectedEnd>monthlyIncome?'#b91c1c':'#1d4ed8'};margin-bottom:4px;">MONTH-END PROJECTION</p>
                    <p style="font-size:18px;font-weight:900;color:${projectedEnd>monthlyIncome?'#dc2626':'#1e40af'};margin:0;">₹${projectedEnd.toLocaleString('en-IN')}</p>
                </div>
            </div>
            <div style="background:var(--line-color);border-radius:16px;padding:16px;margin-bottom:12px;">
                <div style="display:flex;justify-content:space-between;margin-bottom:10px;"><span style="font-size:13px;font-weight:700;color:var(--text-main);">Daily Average</span><span style="font-size:14px;font-weight:900;color:#f59e0b;">₹${Math.round(dailyAvg).toLocaleString('en-IN')}/day</span></div>
                <div style="display:flex;justify-content:space-between;margin-bottom:10px;"><span style="font-size:13px;font-weight:700;color:var(--text-main);">Upcoming EMI</span><span style="font-size:14px;font-weight:900;color:#ef4444;">₹${upcomingEMI.toLocaleString('en-IN')}</span></div>
                <div style="display:flex;justify-content:space-between;"><span style="font-size:13px;font-weight:700;color:var(--text-main);">Expected Savings</span><span style="font-size:14px;font-weight:900;color:${projectedSavings>=0?'#10b981':'#ef4444'};">₹${Math.abs(projectedSavings).toLocaleString('en-IN')}</span></div>
            </div>
            <div style="background:${projectedEnd>monthlyIncome?'#fef2f2':'#f0fdf4'};border-radius:14px;padding:14px;border:1px solid ${projectedEnd>monthlyIncome?'#fca5a5':'#86efac'};">
                <p style="font-size:13px;font-weight:700;color:${projectedEnd>monthlyIncome?'#b91c1c':'#15803d'};margin:0;">${projectedEnd>monthlyIncome?'Kharcha income se zyada hoga! Roz ₹'+Math.round((projectedEnd-monthlyIncome)/daysLeft)+' kam kharchein.':'Budget track par hai. Roz ₹'+Math.round(dailyAvg)+' spend ho raha hai.'}</p>
            </div>
        </div>`,
        showConfirmButton: false, showCloseButton: true, background: 'var(--paper-bg)', color: 'var(--text-main)', width: '92%', maxWidth: '440px'
    });
...
// ── FEATURE NEW-2: Spending Heatmap ──────────────────
function openExpenseHeatmap() {
    const last30 = [];
    const today = new Date();
    for(let i=29; i>=0; i--) {
        ...
        const amt = familyExpenses.filter(e=>e.date===ds).reduce((s,e)=>s+parseFloat(e.amount),0);
        last30.push({date:ds, day:d.getDate(), amt});
    }
    const maxAmt = Math.max(...last30.map(d=>d.amt), 1);
    const cells = last30.map(d=>{
        const intensity = d.amt/maxAmt;
        const bg = d.amt===0?'#f1f5f9':intensity<0.25?'#bbf7d0':intensity<0.5?'#86efac':intensity<0.75?'#f59e0b':'#ef4444';
        const textColor = d.amt===0?'#94a3b8':intensity>0.5?'white':'#1e293b';
        return `<div style="aspect-ratio:1;background:${bg};border-radius:6px;display:flex;align-items:center;justify-content:center;cursor:default;" title="${d.date}: ₹${Math.round(d.amt)}"><span style="font-size:10px;font-weight:800;color:${textColor};">${d.day}</span></div>`;
    }).join('');
    Swal.fire({
        title: 'Spending Heatmap — Last 30 Days',
        html: `<div>
            <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:4px;margin-bottom:14px;">${cells}</div>
            <div style="display:flex;gap:8px;align-items:center;justify-content:center;flex-wrap:wrap;">
                <div style="display:flex;align-items:center;gap:4px;"><div style="width:14px;height:14px;background:#f1f5f9;border-radius:3px;"></div><span style="font-size:11px;color:#64748b;">No spend</span></div>
                <div style="display:flex;align-items:center;gap:4px;"><div style="width:14px;height:14px;background:#86efac;border-radius:3px;"></div><span style="font-size:11px;color:#64748b;">Low</span></div>
                <div style="display:flex;align-items:center;gap:4px;"><div style="width:14px;height:14px;background:#f59e0b;border-radius:3px;"></div><span style="font-size:11px;color:#64748b;">Medium</span></div>
                <div style="display:flex;align-items:center;gap:4px;"><div style="width:14px;height:14px;background:#ef4444;border-radius:3px;"></div><span style="font-size:11px;color:#64748b;">High</span></div>
            </div>
        </div>`,
        confirmButtonText: 'Close', confirmButtonColor: '#7c3aed', background: 'var(--paper-bg)', color: 'var(--text-main)', width: '92%', maxWidth: '440px'
    });
    playSound('click');
}

// ── FEATURE NEW-3: Smart Reminders ──────────────────
let smartRemindersList = JSON.parse(localStorage.getItem('smartReminders')||'[]');
function openSmartReminders() {
    // Auto-generate reminders from data
    const autoReminders = [];
    activeSubs.forEach(s => { if(s.dueDate) autoReminders.push({text:`${s.name} subscription due on ${s.dueDate}th`, amount:s.amount, type:'sub', auto:true}); });
    activeLoans.forEach(l => { if(l.dueDate) autoReminders.push({text:`${l.name} EMI due on ${l.dueDate}th`, amount:l.emi||0, type:'emi', auto:true}); });
    rechargeRecords.slice(0,3).forEach(r => {
        const exp = new Date(r.expiry); const today = new Date();
        const diff = Math.ceil((exp-today)/(1000*60*60*24));
        if(diff<=5&&diff>0) autoReminders.push({text:`${r.name} recharge expiring in ${diff} days!`, amount:r.amount, type:'recharge', auto:true});
    });
    const allReminders = [...autoReminders, ...smartRemindersList];
    const listHTML = allReminders.length > 0 ? allReminders.map((r,i) => `
        <div style="display:flex;align-items:center;gap:12px;padding:12px;background:${r.type==='emi'?'#fef2f2':r.type==='sub'?'#eff6ff':'#fefce8'};border-radius:14px;margin-bottom:8px;">
            <div style="width:36px;height:36px;background:${r.type==='emi'?'#ef4444':r.type==='sub'?'#3b82f6':'#f59e0b'};border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" width="16" height="16"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
            </div>
            <div style="flex:1;"><p style="font-size:13px;font-weight:700;color:var(--text-main);margin:0;">${r.text}</p>${r.amount?`<p style="font-size:11px;color:#64748b;margin:0;">Amount: ₹${r.amount}</p>`:''}</div>
            ${!r.auto?`<button onclick="deleteReminder(${smartRemindersList.indexOf(r)})" style="background:none;border:none;cursor:pointer;color:#ef4444;font-size:16px;">×</button>`:''}
        </div>`).join('') : '<p style="text-align:center;color:#94a3b8;padding:20px;">Koi reminder nahi. Add karo!</p>';
    Swal.fire({
        title: 'Smart Reminders',
        html: `<div style="text-align:left;max-height:60vh;overflow-y:auto;">${listHTML}
            <button onclick="addCustomReminder()" style="width:100%;padding:12px;background:linear-gradient(135deg,#0891b2,#0e7490);color:white;border:none;border-radius:14px;font-weight:800;cursor:pointer;margin-top:10px;">+ Add Custom Reminder</button>
        </div>`,
        showConfirmButton: false, showCloseButton: true, background: 'var(--paper-bg)', color: 'var(--text-main)', width: '90%', maxWidth: '400px'
    });
    playSound('click');
...
function addCustomReminder() {
    Swal.fire({title:'New Reminder',html:`<input id="rem-text" placeholder="Reminder text" style="width:100%;padding:10px;border:2px solid var(--line-color);border-radius:12px;margin-bottom:8px;box-sizing:border-box;background:var(--paper-bg);color:var(--text-main);">`,showCancelButton:true,confirmButtonText:'Add',confirmButtonColor:'#0891b2',background:'var(--paper-bg)',color:'var(--text-main)',preConfirm:()=>{const t=document.getElementById('rem-text').value;if(!t)return Swal.showValidationMessage('Kuch likho!');return{text:t,type:'custom',auto:false};}}).then(r=>{if(r.isConfirmed){smartRemindersList.push(r.value);localStorage.setItem('smartReminders',JSON.stringify(smartRemindersList));openSmartReminders();}});
}
function deleteReminder(i){smartRemindersList.splice(i,1);localStorage.setItem('smartReminders',JSON.stringify(smartRemindersList));openSmartReminders();}

// ── FEATURE NEW-4: Recurring Detector ────────────────
function openRecurringDetector() {
    const descMap = {};
    familyExpenses.forEach(e => {
        const key = e.description.toLowerCase().trim();
        if(!descMap[key]) descMap[key] = {count:0, total:0, lastDate:'', category:e.category};
        descMap[key].count++;
        descMap[key].total += parseFloat(e.amount);
        if(e.date > descMap[key].lastDate) descMap[key].lastDate = e.date;
    });
    const recurring = Object.entries(descMap).filter(([k,v])=>v.count>=2).sort((a,b)=>b[1].count-a[1].count).slice(0,10);
    const listHTML = recurring.length > 0 ? recurring.map(([name,data]) => `
        <div style="display:flex;align-items:center;gap:10px;padding:12px;background:var(--line-color);border-radius:14px;margin-bottom:8px;">
            <div style="width:36px;height:36px;background:linear-gradient(135deg,#b45309,#92400e);border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:white;font-size:13px;font-weight:800;">${data.count}x</div>
            <div style="flex:1;"><p style="font-size:13px;font-weight:800;color:var(--text-main);margin:0;text-transform:capitalize;">${name}</p><p style="font-size:11px;color:#64748b;margin:0;">${data.category} | Total: ₹${data.total.toLocaleString('en-IN')}</p></div>
            <span style="font-size:12px;font-weight:900;color:#b45309;">₹${Math.round(data.total/data.count)}/avg</span>
        </div>`).join('') : '<p style="text-align:center;color:#94a3b8;padding:20px;">2+ expenses ke baad patterns dikhenge.</p>';
    Swal.fire({
        title: 'Recurring Expense Detector',
        html: `<div style="text-align:left;"><p style="font-size:12px;color:#64748b;margin-bottom:12px;">Frequently repeated kharche — inhe subscription ya fix karo.</p>${listHTML}</div>`,
        confirmButtonText:'Samajh Gaya',confirmButtonColor:'#b45309',background:'var(--paper-bg)',color:'var(--text-main)',width:'90%',maxWidth:'400px'
    });
    playSound('click');
}

// ── FEATURE NEW-5: Currency Converter ────────────────
async function openCurrencyConverter() {
    Swal.fire({
        title: 'Currency Converter',
        html: `<div style="text-align:left;">
            <input type="number" id="cc-amount" placeholder="Amount (₹)" value="${monthlyIncome||1000}" style="width:100%;padding:10px;border:2px solid var(--line-color);border-radius:12px;margin-bottom:10px;box-sizing:border-box;background:var(--paper-bg);color:var(--text-main);">
            <select id="cc-to" style="width:100%;padding:10px;border:2px solid var(--line-color);border-radius:12px;margin-bottom:10px;background:var(--paper-bg);color:var(--text-main);font-weight:700;">
            ...
            <div id="cc-result" style="background:var(--line-color);border-radius:14px;padding:14px;text-align:center;display:none;"></div>
            <button onclick="doConvert()" style="width:100%;padding:12px;background:linear-gradient(135deg,#15803d,#166534);color:white;border:none;border-radius:14px;font-weight:800;cursor:pointer;margin-top:10px;">Convert</button>
        </div>`,
...
async function doConvert() {
    const amt = parseFloat(document.getElementById('cc-amount').value)||0;
    const to = document.getElementById('cc-to').value;
    const rates = {USD:0.012,EUR:0.011,GBP:0.0095,AED:0.044,SGD:0.016,SAR:0.045,JPY:1.78};
    const rate = rates[to]||0.012;
    const result = (amt*rate).toFixed(2);
    const el = document.getElementById('cc-result');
    if(el){el.style.display='block';el.innerHTML=`<p style="font-size:12px;color:#64748b;margin-bottom:4px;">₹${amt.toLocaleString('en-IN')} =</p><p style="font-size:26px;font-weight:900;color:var(--text-main);margin:0;">${to} ${parseFloat(result).toLocaleString()}</p><p style="font-size:10px;color:#64748b;margin-top:4px;">Rate: 1 INR = ${rate} ${to} (approx)</p>`;}
}

// ── FEATURE NEW-6: Daily Limit Alert ─────────────────
function openDailyLimit() {
    const currentLimit = parseFloat(localStorage.getItem('dailyLimit')||'0');
    const todaySpend = familyExpenses.filter(e=>e.date===todayDateString).reduce((s,e)=>s+parseFloat(e.amount),0);
    Swal.fire({
        title: 'Daily Limit Alert',
        html:`<div style="text-align:left;">
            ${todaySpend>0?`<div style="background:${todaySpend>currentLimit&&currentLimit>0?'#fef2f2':'#f0fdf4'};border-radius:14px;padding:14px;margin-bottom:14px;border:1px solid ${todaySpend>currentLimit&&currentLimit>0?'#fca5a5':'#86efac'};"><p style="font-size:11px;font-weight:800;color:#64748b;margin-bottom:4px;">AAJ KA KHARCHA</p><p style="font-size:24px;font-weight:900;color:${todaySpend>currentLimit&&currentLimit>0?'#dc2626':'#15803d'};margin:0;">₹${todaySpend.toLocaleString('en-IN')}</p>${currentLimit>0?`<p style="font-size:12px;font-weight:700;color:${todaySpend>currentLimit?'#b91c1c':'#166534'};margin-top:4px;">${todaySpend>currentLimit?'Limit se ₹'+(todaySpend-currentLimit)+' zyada!':'₹'+(currentLimit-todaySpend)+' limit mein baaki'}</p>`:''}</div>`:'' }
            <label style="font-size:12px;font-weight:800;color:#64748b;display:block;margin-bottom:6px;">Daily Limit Set Karo (₹):</label>
            <input type="number" id="daily-limit-input" placeholder="e.g., 500" value="${currentLimit||''}" style="width:100%;padding:10px;border:2px solid var(--line-color);border-radius:12px;box-sizing:border-box;background:var(--paper-bg);color:var(--text-main);">
        </div>`,
        showCancelButton:true,confirmButtonText:'Save Limit',confirmButtonColor:'#be123c',background:'var(--paper-bg)',color:'var(--text-main)',width:'90%',maxWidth:'380px',
        preConfirm:()=>{const v=parseFloat(document.getElementById('daily-limit-input').value);if(!v||v<=0)return Swal.showValidationMessage('Valid amount dalo');return v;}
    }).then(r=>{if(r.isConfirmed){localStorage.setItem('dailyLimit',r.value);Swal.fire({toast:true,position:'top',icon:'success',title:'Daily limit ₹'+r.value+' set ho gaya!',timer:2000,showConfirmButton:false});}});
    playSound('click');
}

// ── FEATURE NEW-7: Financial Health Report ───────────
function openHealthReport() {
    const fm = todayDateString.slice(0,7);
    const monthExp = familyExpenses.filter(e=>e.date&&e.date.startsWith(fm)).reduce((s,e)=>s+parseFloat(e.amount),0);
    const savings = monthlyIncome - monthExp;
    const totalInvest = investments.reduce((s,i)=>s+(i.amount||0),0);
    const totalDebt = activeLoans.reduce((s,l)=>s+(l.principal||0),0);
    const totalEMI = activeLoans.reduce((s,l)=>s+(l.emi||0),0);
    const criteria = [
        {label:'Savings Rate',value:monthlyIncome>0?Math.round((savings/monthlyIncome)*100):0,target:20,unit:'%',good:v=>v>=20,tip:'20% income bachana chahiye'},
        {label:'EMI to Income',value:monthlyIncome>0?Math.round((totalEMI/monthlyIncome)*100):0,target:30,unit:'%',good:v=>v<=30,tip:'EMI income ke 30% se kam ho'},
        {label:'Emergency Fund',value:totalInvest>0?Math.round((totalInvest/(monthExp*6))*100):0,target:100,unit:'%',good:v=>v>=100,tip:'6 mahine ki expenses invest kar'},
        {label:'Debt Ratio',value:totalDebt>0&&monthlyIncome>0?Math.round((totalDebt/monthlyIncome)*100):0,target:300,unit:'%',good:v=>v<=300,tip:'Total debt 3 saal ki income se kam ho'},
    ];
    const totalScore = Math.round(criteria.reduce((s,c)=>s+(c.good(c.value)?25:Math.min(25,(c.value/c.target)*25)),0));
    const scoreColor = totalScore>=75?'#10b981':totalScore>=50?'#f59e0b':'#ef4444';
    const criteriaHTML = criteria.map(c=>`<div style="margin-bottom:12px;"><div style="display:flex;justify-content:space-between;margin-bottom:4px;"><span style="font-size:13px;font-weight:700;color:var(--text-main);">${c.label}</span><span style="font-size:13px;font-weight:900;color:${c.good(c.value)?'#10b981':'#ef4444'};">${c.value}${c.unit}</span></div><div style="background:var(--line-color);border-radius:8px;height:8px;overflow:hidden;"><div style="height:100%;width:${Math.min(100,c.good(c.value)?100:(c.value/c.target)*100)}%;background:${c.good(c.value)?'#10b981':'#ef4444'};border-radius:8px;transition:width 0.5s;"></div></div><p style="font-size:10px;color:#64748b;margin-top:3px;">${c.tip}</p></div>`).join('');
    Swal.fire({
        title:'',html:`<div style="text-align:left;">
            <div style="text-align:center;margin-bottom:16px;"><p style="font-size:60px;font-weight:900;color:${scoreColor};margin:0;">${totalScore}</p><p style="font-size:12px;font-weight:800;color:${scoreColor};">/100 — ${totalScore>=75?'Excellent!':totalScore>=50?'Average':'Needs Work'}</p></div>
            ${criteriaHTML}
        </div>`,
        confirmButtonText:'Dekh Liya',confirmButtonColor:scoreColor,background:'var(--paper-bg)',color:'var(--text-main)',width:'90%',maxWidth:'420px'
    });
    playSound('click');
}

// ── FEATURE NEW-8: Category Budgets ──────────────────
function openCategoryBudgets() {
    const categories = ['Ration','Medical','Petrol','Bills','Food','Shopping','Travel','Other'];
    let catBudgets = JSON.parse(localStorage.getItem('categoryBudgets')||'{}');
    const fm = todayDateString.slice(0,7);
    const catSpent = {};
    familyExpenses.filter(e=>e.date&&e.date.startsWith(fm)).forEach(e=>{ catSpent[e.category]=(catSpent[e.category]||0)+parseFloat(e.amount); });
    const listHTML = categories.map(cat=>{
        ...
        return `<div style="margin-bottom:12px;background:var(--line-color);border-radius:14px;padding:12px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                <span style="font-size:13px;font-weight:800;color:var(--text-main);">${cat}</span>
                <div style="display:flex;align-items:center;gap:6px;">
                    <input type="number" placeholder="Budget" value="${budget||''}" onchange="setCatBudget('${cat}',this.value)" style="width:80px;padding:4px 8px;border:1.5px solid var(--line-color);border-radius:8px;font-size:12px;font-weight:700;background:var(--paper-bg);color:var(--text-main);">
                    <span style="font-size:12px;font-weight:900;color:${spent>0?'#ef4444':'#64748b'};">₹${spent.toLocaleString('en-IN')}</span>
                </div>
            </div>
            ${budget>0?`<div style="background:rgba(0,0,0,0.08);border-radius:6px;height:6px;overflow:hidden;"><div style="height:100%;width:${pct}%;background:${barColor};border-radius:6px;transition:width 0.4s;"></div></div><p style="font-size:10px;color:${barColor};font-weight:700;margin-top:3px;">${pct}% used</p>`:''}
        </div>`;
    }).join('');
    Swal.fire({
        title:'Category Budgets',
        html:`<div style="text-align:left;max-height:65vh;overflow-y:auto;">${listHTML}</div>`,
...
function setCatBudget(cat, val) {
    let catBudgets = JSON.parse(localStorage.getItem('categoryBudgets')||'{}');
    if(val&&parseFloat(val)>0) catBudgets[cat]=parseFloat(val); else delete catBudgets[cat];
    localStorage.setItem('categoryBudgets',JSON.stringify(catBudgets));
}

// ── FEATURE NEW-9: Expense Diary ─────────────────────
let diaryEntries = JSON.parse(localStorage.getItem('expenseDiary')||'[]');
function openExpenseDiary() {
    const listHTML = diaryEntries.slice(0,10).map((entry,i)=>`
        <div style="background:var(--line-color);border-radius:16px;padding:14px;margin-bottom:10px;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;">
                <span style="font-size:11px;font-weight:800;color:#64748b;">${entry.date}</span>
                <button onclick="deleteDiaryEntry(${i})" style="background:none;border:none;color:#ef4444;cursor:pointer;font-size:14px;line-height:1;">×</button>
            </div>
            <p style="font-size:14px;font-weight:600;color:var(--text-main);margin:0;line-height:1.5;">${entry.text}</p>
            ${entry.mood?`<span style="font-size:11px;color:#64748b;margin-top:4px;display:block;">Mood: ${entry.mood}</span>`:''}
        </div>`).join('') || '<p style="text-align:center;color:#94a3b8;padding:20px;">Koi diary entry nahi. Apne finance ke bare mein likhna shuru karo!</p>';
    Swal.fire({
        title:'My Expense Diary',
        html:`<div style="text-align:left;max-height:60vh;overflow-y:auto;">${listHTML}
            <button onclick="addDiaryEntry()" style="width:100%;padding:12px;background:linear-gradient(135deg,#4338ca,#312e81);color:white;border:none;border-radius:14px;font-weight:800;cursor:pointer;margin-top:10px;">+ New Entry</button>
        </div>`,
        showConfirmButton:false,showCloseButton:true,background:'var(--paper-bg)',color:'var(--text-main)',width:'90%',maxWidth:'420px'
    });
    playSound('click');
...
function addDiaryEntry(){
    Swal.fire({
        ...
        html:`<textarea id="diary-text" placeholder="Aaj kaise tha kharcha? Koi special plan? Mann ki baat..." style="width:100%;min-height:120px;padding:10px;border:2px solid var(--line-color);border-radius:12px;box-sizing:border-box;background:var(--paper-bg);color:var(--text-main);font-size:14px;resize:none;"></textarea>
        <select id="diary-mood" style="width:100%;margin-top:8px;padding:10px;border:2px solid var(--line-color);border-radius:12px;background:var(--paper-bg);color:var(--text-main);font-weight:700;">
            <option value="">Select Mood</option>
            <option value="Happy - Paise bache">Happy - Paise bache</option>
            <option value="Worried - Zyada kharcha">Worried - Zyada kharcha</option>
            <option value="Motivated - Saving ke liye">Motivated - Saving ke liye</option>
            <option value="Neutral - Normal din">Neutral - Normal din</option>
        </select>`,
        showCancelButton:true,confirmButtonText:'Save Entry',confirmButtonColor:'#4338ca',background:'var(--paper-bg)',color:'var(--text-main)',
        preConfirm:()=>{const t=document.getElementById('diary-text').value;if(!t||t.trim().length<5)return Swal.showValidationMessage('Kuch zyada likho!');return{text:t.trim(),mood:document.getElementById('diary-mood').value,date:todayDateString};}
    }).then(r=>{if(r.isConfirmed){diaryEntries.unshift(r.value);localStorage.setItem('expenseDiary',JSON.stringify(diaryEntries));openExpenseDiary();playSound('success');}});
}
function deleteDiaryEntry(i){diaryEntries.splice(i,1);localStorage.setItem('expenseDiary',JSON.stringify(diaryEntries));openExpenseDiary();}

// ── FEATURE NEW-10: Income Tax Estimator ─────────────
function openTaxEstimator() {
    ...
    const slabDetails = slabs.map(slab => {
        if(annualIncome <= slab.min) return null;
        const taxable = Math.min(annualIncome, slab.max) - slab.min;
        const slabTax = taxable * slab.rate / 100;
        totalTax += slabTax;
        return {label:slab.label, taxable:Math.round(taxable), tax:Math.round(slabTax), rate:slab.rate};
    }).filter(Boolean);
    ...
    Swal.fire({
        title:'',
        html:`<div style="text-align:left;">
            <div style="background:linear-gradient(135deg,#065f46,#064e3b);border-radius:20px;padding:18px;margin-bottom:16px;color:white;">
                <p style="font-size:11px;color:#6ee7b7;font-weight:800;text-transform:uppercase;margin-bottom:4px;">Income Tax Estimator (New Regime)</p>
                <p style="font-size:26px;font-weight:900;margin:0;">₹${totalTax.toLocaleString('en-IN')}</p>
                <p style="font-size:12px;color:#a7f3d0;margin-top:4px;">Annual Tax | Effective Rate: ${effectiveRate}% | Monthly: ₹${monthlyTax.toLocaleString('en-IN')}</p>
            </div>
            <p style="font-size:12px;font-weight:800;color:#64748b;margin-bottom:8px;text-transform:uppercase;">Slab Breakdown</p>
            ${slabDetails.map(s=>`<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--line-color);"><div><span style="font-size:12px;font-weight:700;color:var(--text-main);">${s.label}</span><span style="font-size:10px;color:#64748b;margin-left:6px;">@${s.rate}%</span></div><span style="font-size:13px;font-weight:800;color:${s.tax>0?'#ef4444':'#10b981'};">${s.tax>0?'₹'+s.tax.toLocaleString('en-IN'):'NIL'}</span></div>`).join('')}
            <div style="display:flex;justify-content:space-between;padding:8px 0;"><span style="font-size:12px;font-weight:700;color:var(--text-main);">4% Health & Education Cess</span><span style="font-size:13px;font-weight:800;color:#ef4444;">₹${cess.toLocaleString('en-IN')}</span></div>
            <p style="font-size:10px;color:#94a3b8;margin-top:10px;">*Estimate only. Consult CA for accurate computation. Based on Annual Income ₹${annualIncome.toLocaleString('en-IN')}.</p>
        </div>`,
        confirmButtonText:'Samajh Gaya',confirmButtonColor:'#065f46',background:'var(--paper-bg)',color:'var(--text-main)',width:'90%',maxWidth:'420px'
    });
    playSound('click');
}
