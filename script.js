...
auth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
        ...
        let userName = user.email.split("@")[0]; // Email ka pehla hissa naam ban jayega
...
function loginWithEmail() {
    ...
    if (!email || password.length < 6) {
        return Swal.fire('Oops!', 'Sahi email aur kam se kam 6 akshar ka password daalein.', 'warning');
    }

    if(loginStatus) {
        loginStatus.style.display = 'block';
        loginStatus.innerText = "Login kar rahe hain... ⏳";
    }
...
function registerWithEmail() {
    ...
    if (!email || password.length < 6) {
        return Swal.fire('Oops!', 'Naya account banane ke liye sahi email aur password daalein.', 'warning');
    }

    if(loginStatus) {
        loginStatus.style.display = 'block';
        loginStatus.innerText = "Naya account bana rahe hain... ⏳";
    }
