...
// ==========================================
// 📳 SOUND & NATIVE HAPTIC VIBRATION (Phase 19)
// ==========================================
...
function playSound(type) { 
    if(navigator.vibrate) navigator.vibrate(type === 'success' ? [30, 50, 30] : 30); // 📳 HAPTIC FEEDBACK
...
// ==========================================
// 🔄 PULL TO REFRESH NATIVE GESTURE (Phase 19)
// ==========================================
...
document.addEventListener('touchend', e => {
    ...
    if(pEndY > pStartY + 150 && window.scrollY === 0 && currentUser) {
        ...
        setTimeout(() => { 
            ...
            pRef.innerText = '✅ Synced!'; playSound('success');
            setTimeout(() => pRef.style.display = 'none', 1000);
        }, 1000);
    }
});
...
// ==========================================
// 🛡️ USER PROFILE, XP & SAFE CODE
// ==========================================
function updateProfileName() { const currentName = customDisplayName || (currentUser && currentUser.email ? currentUser.email.split("@")[0] : "User"); Swal.fire({ title: 'Apna Naam Likhein', input: 'text', inputValue: currentName, showCancelButton: true, confirmButtonText: 'Save', confirmButtonColor: '#2563eb' }).then((result) => { if (result.isConfirmed) { customDisplayName = result.value.trim(); saveToCloud(); updateGreetingName(); Swal.fire('Saved!', '', 'success'); } }); playSound('click'); }
...
// ==========================================
// 💰 HISAAB SECTION & ✌️ DOUBLE TAP TO EDIT (Phase 19)
// ==========================================
...
function importBankCSV() {
    ...
    reader.onload = function(e) {
        ...
        fileInput.value = "";
    }; reader.readAsText(fileInput.files[0]); playSound('click');
}

function setBudget() { Swal.fire({ title: 'Monthly Budget', input: 'number', inputValue: budgetLimit, showCancelButton: true }).then((result) => { if (result.isConfirmed && result.value > 0) { budgetLimit = result.value; saveToCloud(); renderHistoryWithSkeleton(); } }); playSound('click'); }
function setIncome() { Swal.fire({ title: 'Is Mahine Ki Kamai (Income)', input: 'number', inputValue: monthlyIncome, showCancelButton: true }).then((result) => { if (result.isConfirmed && result.value >= 0) { monthlyIncome = parseFloat(result.value); saveToCloud(); updateHisabUI(); } }); playSound('click'); }
...
function updateHisabUI() {
    ...
    if(filteredExpenses.length === 0) {
        ...
        filteredExpenses.forEach((item) => {
            ...
            // ✌️ NAYA: DOUBLE TAP TO EDIT (Phase 19)
            li.setAttribute('ondblclick', `editExpense(${originalIndex})`);
            li.style.cursor = 'pointer'; // to indicate it's clickable
...
function backupData() { 
    ...
    const dlAnchorElem = document.createElement('a'); dlAnchorElem.setAttribute("href", dataStr); dlAnchorElem.setAttribute("download", "GharManager_Encrypted_Backup.txt"); dlAnchorElem.click(); playSound('success');
}
    ...
    reader.onload = async function(e) { 
        try { 
            ...
            if (data.expenses) { 
                ...
                await saveToCloud(); Swal.fire('Restored!', 'Aapka purana data wapas aa gaya hai! ✅', 'success'); loadCloudData(currentUser.uid); playSound('success');
...
function startVoice() { 
    ...
    recognition.onerror = () => { btn.innerText = "🎤"; Swal.fire('Error', 'Awaz clear nahi aayi!', 'error'); }; recognition.start(); playSound('click');
}
