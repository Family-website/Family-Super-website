...
function checkRole(name) { 
    ...
    if(n.includes('aditya') || n.includes('papa')) { userRole = 'Admin'; document.getElementById('role-badge').innerText = '👑 Admin'; document.getElementById('role-badge').style.background = '#fef08a'; document.getElementById('role-badge').style.color = '#b45309'; }
    else { userRole = 'Member'; document.getElementById('role-badge').innerText = '👤 Member'; document.getElementById('role-badge').style.background = '#e2e8f0'; document.getElementById('role-badge').style.color = '#475569'; }
}
...
function loadCloudData(uid) {
    ...
    db.collection('familyData').doc(uid).onSnapshot((doc) => {
        if (doc.exists) {
            ...
            updateHisabUI(); updateDudhUI(); updateRationUI(); updateInvestUI(); updateLoanUI(); updateSubsUI(); updateGreetingName(); checkRole(customDisplayName || (currentUser?currentUser.email:"")); updateChallengeUI(); checkStreak(); updateToDoUI();
        } else { updateHisabUI(); }
    });
}
...
async function saveToCloud() { 
    ...
    await db.collection('familyData').doc(currentUser.uid).set({ expenses: familyExpenses, dudh: dudhRecords, ration: rationItems, investments: investments, loans: activeLoans, subscriptions: activeSubs, budget: budgetLimit, displayName: customDisplayName, income: monthlyIncome, xp: userXP, challengeDays: challengeDays, dailyStreak: dailyStreak, lastLoginDate: lastLoginDate, todoItems: todoItems, dreamGoal: dreamGoal }, { merge: true }); showSyncSuccess(); 
}

async function syncOldLocalData() { 
    if(isOffline) return;
    const localExp = JSON.parse(localStorage.getItem('familyExpenses')); const localDudh = JSON.parse(localStorage.getItem('dudhRecords')); const localRation = JSON.parse(localStorage.getItem('rationItems')); let dataChanged = false; 
...
function openProfile() {
    try {
        ...
        if(scoreBar && scoreText && scoreMsg) { scoreBar.style.width = `${score}%`; scoreText.innerText = `${score}/100`; }
...
// ==========================================
// 💰 HISAAB SECTION & SHIMMER EFFECT (Phase 20)
// ==========================================
...
function setBudget() { if(userRole !== 'Admin') return Swal.fire('Access Denied', 'Sirf Admin (Papa/Aditya) hi budget badal sakte hain', 'error'); Swal.fire({ title: 'Monthly Budget', input: 'number', inputValue: budgetLimit, showCancelButton: true }).then((result) => { if (result.isConfirmed && result.value > 0) { budgetLimit = result.value; saveToCloud(); renderHistoryWithSkeleton(); } }); playSound('click'); }
function setIncome() { if(userRole !== 'Admin') return Swal.fire('Access Denied', 'Sirf Admin (Papa/Aditya) hi income add kar sakte hain', 'error'); Swal.fire({ title: 'Is Mahine Ki Kamai', input: 'number', inputValue: monthlyIncome, showCancelButton: true }).then((result) => { if (result.isConfirmed && result.value >= 0) { monthlyIncome = parseFloat(result.value); saveToCloud(); updateHisabUI(); } }); playSound('click'); }

function renderHistoryWithSkeleton() { 
    const list = document.getElementById('history-list'); if(!list) return; 
    list.innerHTML = `<div class="skeleton-box" style="height:60px; margin-bottom:10px;"></div><div class="skeleton-box" style="height:60px; margin-bottom:10px;"></div><div class="skeleton-box" style="height:60px;"></div>`; 
    setTimeout(updateHisabUI, 600); 
}
...
function updateHisabUI() {
    ...
    renderCalendar(filteredExpenses, filterMonth); renderCategoryChart(categoryTotals); renderMemberChart(memberTotals); renderTrendChart(filteredExpenses); applyLanguageUI();
}
