...
function verifyPin() {
    ...
    if(pin === savedPin) {
        ...
        Swal.fire('Galat PIN ❌', 'Kripya sahi PIN daalein', 'error');
        document.getElementById('pin-input').value = "";
    }
}

// ==========================================
// ☁️ 3. CLOUD DATA SYNC & GLOBALS (ALL PHASES)
// ==========================================
...
let lastLoginDate = ""; 
let todoItems = [];
let dreamGoal = { name: "No Goal", target: 0 };
...
function checkStreak() {
    ...
    if(lastLoginDate !== today) {
        ...
        if(lastLoginDate === yesterday) { dailyStreak += 1; } 
        else if(lastLoginDate !== "") { dailyStreak = 1; } 
        else { dailyStreak = 1; }
        lastLoginDate = today; saveToCloud();
        
        if(dailyStreak > 1) {
            Swal.fire({ title: '🔥 Streak Maintained!', text: `Tum lagatar ${dailyStreak} dinon se app use kar rahe ho! Bonus XP!`, icon: 'success', toast: true, position: 'top-end', timer: 3000, showConfirmButton: false });
            gainXP(5);
        }
    }
    let streakEl = document.getElementById('daily-streak');
    if(streakEl) streakEl.innerText = `🔥 ${dailyStreak} Day Streak`;
}
...
function setGoal() {
    Swal.fire({
        title: 'Set Your Dream Goal 🎯',
        html: '<input id="swal-goal-name" class="swal2-input" placeholder="Goal Name (e.g. New Phone, Bike)">' +
              '<input id="swal-goal-target" type="number" class="swal2-input" placeholder="Target Amount (₹)">',
        focusConfirm: false,
        preConfirm: () => { return { name: document.getElementById('swal-goal-name').value, target: parseFloat(document.getElementById('swal-goal-target').value) } }
    }).then((result) => {
        if(result.isConfirmed && result.value.target > 0) {
            dreamGoal = { name: result.value.name || 'My Dream', target: result.value.target };
            saveToCloud(); openProfile(); playSound('success'); Swal.fire('Set!', 'Naya goal set ho gaya hai!', 'success');
        }
    });
}
...
function updateToDoUI() {
    ...
    todoItems.forEach((task, index) => {
        ...
        li.style.background = 'transparent'; li.style.borderBottom = '1px dashed #fde68a'; li.style.borderRadius = '0'; li.style.padding = '8px 0'; li.style.marginBottom = '0';
        li.innerHTML = `
            <div style="display:flex; align-items:center; cursor:pointer; flex: 1;" onclick="toggleToDo(${index})">
                <input type="checkbox" ${task.done ? 'checked' : ''} style="width:18px; height:18px; margin-right:10px; accent-color:#f59e0b; pointer-events:none;">
                <span style="font-size:14px; font-weight:700; color:#92400e; text-decoration:${task.done ? 'line-through' : 'none'}; opacity:${task.done ? '0.5' : '1'}">${task.text}</span>
            </div>
            <button onclick="deleteToDo(${index})" style="background:none; border:none; font-size:16px; cursor:pointer; opacity:0.6;">❌</button>
        `;
        list.appendChild(li);
    });
}
...
function autoDarkMode() {
    ...
    if(hour >= 18 || hour < 6) { if(localStorage.getItem('appTheme') === 'default' || !localStorage.getItem('appTheme')) applyTheme('night'); }
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if(descInput) {
        descInput.addEventListener('input', function(e) {
            ...
            let cat = document.getElementById('expense-category');
...
function updateHisabUI() {
    ...
    if(bar) {
        ...
        if(warning) {
            warning.innerHTML = `📊 Usage: <b>${budgetPercent}%</b> | ${budgetPercent >= 80 ? '⚠️ Budget limit ke paas ho!' : '✅ Budget control mein hai.'}`;
            warning.style.display = 'block';
            warning.style.color = budgetPercent >= 80 ? '#e74c3c' : '#10b981';
        }
    }
    ...
    if(monthlyIncome > 0 && planner) {
        ...
        document.getElementById('rule-needs').innerText = `₹${Math.round(monthlyIncome * 0.50)}`;
        document.getElementById('rule-wants').innerText = `₹${Math.round(monthlyIncome * 0.30)}`;
        document.getElementById('rule-saves').innerText = `₹${Math.round(monthlyIncome * 0.20)}`;
    } else if(planner) { planner.style.display = 'none'; }
...
// ==========================================
// 🏦 6. THE DEBT MANAGER (ACTIVE EMI SYSTEM)
// ==========================================
let tempLoanData = null;

function calculateEMI() {
    const name = document.getElementById('emi-name').value || 'My Loan'; const p = parseFloat(document.getElementById('emi-principal').value); const r = parseFloat(document.getElementById('emi-rate').value) / 12 / 100; const n = parseFloat(document.getElementById('emi-time').value); const dueDate = parseInt(document.getElementById('emi-due-date').value) || 5;
    if (isNaN(p) || isNaN(r) || isNaN(n) || p <= 0 || n <= 0) return Swal.fire('Galti', 'Sahi details bhariye!', 'error');
    const emi = (p * r * Math.pow(1 + r, n)) / (Math.pow(1 + r, n) - 1); const totalAmount = emi * n; const totalInterest = totalAmount - p;
    ...
    document.getElementById('emi-break-principal').innerText = Math.round(p); document.getElementById('emi-break-interest').innerText = Math.round(totalInterest);
    let pPercent = (p / totalAmount) * 100; document.getElementById('emi-break-bar').style.width = `${pPercent}%`;
    playSound('click');
}

function saveLoan() {
    if(!tempLoanData) return; activeLoans.push(tempLoanData); saveToCloud(); updateLoanUI(); tempLoanData = null;
    Swal.fire('Saved! 🏦', 'Yeh loan list me add ho gaya hai.', 'success'); document.getElementById('emi-result').style.display = 'none';
    document.getElementById('emi-name').value = ''; document.getElementById('emi-principal').value = ''; document.getElementById('emi-rate').value = ''; document.getElementById('emi-time').value = '';
}

function updateLoanUI() {
    const list = document.getElementById('loan-list'); if(!list) return; list.innerHTML = '';
    activeLoans.forEach((loan, index) => {
        let percentPaid = (loan.monthsPaid / loan.time) * 100; let isComplete = loan.monthsPaid >= loan.time;
        const li = document.createElement('li'); li.style.flexDirection = 'column'; li.style.alignItems = 'stretch'; li.style.borderLeft = isComplete ? "4px solid #10b981" : "4px solid #f472b6";
        li.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                <div style="display: flex; flex-direction: column;"><strong style="font-size: 16px;">${loan.name}</strong><span style="font-size: 11px; color: #64748b;">Due: Every ${loan.dueDate}th | EMI: ₹${loan.emi}</span></div>
                <button class="action-btn delete" onclick="deleteLoan(${index})">🗑️</button>
            </div>
            <div style="display: flex; justify-content: space-between; font-size: 11px; font-weight: bold; color: #1e293b; margin-bottom: 4px;"><span>Paid: ${loan.monthsPaid}/${loan.time} Mnth</span><span style="color: ${isComplete ? '#10b981' : '#f59e0b'}">${isComplete ? 'Loan Clear! 🎉' : Math.round(percentPaid)+'% Done'}</span></div>
            <div style="width:100%; background: var(--line-color); border-radius:10px; height:6px; overflow:hidden; margin-bottom: 10px;"><div style="height:100%; width:${percentPaid}%; background: ${isComplete ? '#10b981' : '#f472b6'}; transition: width 0.5s;"></div></div>
...
function payEMI(index) {
    let loan = activeLoans[index]; if(loan.monthsPaid >= loan.time) return;
    Swal.fire({ title: 'Pay EMI?', text: `Kya tum ${loan.name} ka ₹${loan.emi} Hisaab me add karna chahte ho?`, icon: 'question', showCancelButton: true, confirmButtonText: 'Haan, Pay Karo' }).then(async (result) => {
        if (result.isConfirmed) {
            ...
            familyExpenses.push(autoExpense); loan.monthsPaid += 1; gainXP(20); playSound('success');
            if(loan.monthsPaid >= loan.time) { if(typeof confetti !== 'undefined') confetti({ particleCount: 150, spread: 80, origin: { y: 0.6 } }); Swal.fire('Mubarak Ho! 🎉', `Tumhara "${loan.name}" poori tarah se chukta ho gaya hai!`, 'success'); } 
            else { Swal.fire('EMI Paid ✅', 'Hisaab mein add ho gaya hai.', 'success'); }
...
function addInvestment() {
    const type = document.getElementById('invest-type').value; const amt = parseFloat(document.getElementById('invest-amount').value); const date = document.getElementById('invest-date').value || todayDateString;
    ...
    document.getElementById('invest-amount').value = ''; playSound('success'); Swal.fire('Great!', 'Investment add ho gaya!', 'success');
}
...
function updateDudhUI() {
    ...
    dudhRecords.sort((a, b) => new Date(b.date) - new Date(a.date)).forEach((record, index) => {
...
function exportToExcel() {
    ...
    if(dataToExport.length === 0) return Swal.fire('Khali hai!', 'Is mahine koi kharcha nahi hai.', 'info');
    ...
    link.setAttribute("href", url); link.setAttribute("download", `GharManager_Excel_${filterMonth}.csv`); link.style.visibility = 'hidden'; document.body.appendChild(link); link.click(); document.body.removeChild(link); playSound('success'); Swal.fire('Downloaded! 📊', 'Excel file download ho gayi hai.', 'success');
}

// ==========================================
// 👤 10. USER PROFILE & ADVANCED ANALYTICS
// ==========================================
function openProfile() {
    ...
    if(lList) {
        ...
        sortedMembers.forEach((mem, idx) => {
            let medal = idx < 3 ? medals[idx] : '😎'; const li = document.createElement('li');
            ...
            li.innerHTML = `<span style="font-weight:bold; color:#b45309;">${medal} ${mem.name}</span> <span style="font-weight:900; color:#d97706;">₹${mem.amount}</span>`; lList.appendChild(li);
        });
    }
...
function sendWhatsAppReminder(name, amount, type) { let msg = `*GharManager Alert 🔔* \n\nHello ${name}, \nTumhara ${type} ka *₹${amount}* baaki hai. Kripya time par jama kar dein! \n\n_Auto-generated from GharManager App_`; let whatsappUrl = `https://wa.me/?text=${encodeURIComponent(msg)}`; window.open(whatsappUrl, '_blank'); }

// ==========================================
// 🤖 11. LOCAL AI FINANCIAL ASSISTANT & VOICE
// ==========================================
function startVoice() { const recognition = new (window.SpeechRecognition || window.webkitSpeechRecognition)(); recognition.lang = 'hi-IN'; const btn = document.getElementById('mic-btn'); btn.innerText = "🛑"; recognition.onresult = (event) => { document.getElementById('description').value = event.results[0][0].transcript; btn.innerText = "🎤"; playSound('click'); }; recognition.onerror = () => { btn.innerText = "🎤"; Swal.fire('Error', 'Awaz clear nahi aayi!', 'error'); }; recognition.start(); }
