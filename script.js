...
// 🛡️ 11. THE REAL GEMINI AI API (LATEST 2.0 MODEL + FIXES)
async function sendToRealAI() {
    ...
    try {
        // 🔥 ULTIMATE FIX: Using latest 2.0 Flash model
        ...
        if (!response.ok) {
            ...
            chatBox.innerHTML += `<div style="margin:5px 0; text-align:left;"><span style="background:#fee2e2; color:#991b1b; padding:8px 12px; border-radius:16px 16px 16px 0; display:inline-block; font-size:11px;"><b>API Error:</b> ${errMsg}</span></div>`;
            chatBox.scrollTop = chatBox.scrollHeight;
            return;
        }
        ...
        if(document.getElementById(loadingId)) document.getElementById(loadingId).remove();
        chatBox.innerHTML += `<div style="margin:5px 0; text-align:left;"><span style="background:#fee2e2; color:#991b1b; padding:8px 12px; border-radius:16px 16px 16px 0; display:inline-block; font-size:11px;">Internet ya Network Error! Check console.</span></div>`;
...
function openAIBottomSheet() {
    Swal.fire({
        ...
        html: `<div style="display:flex; flex-direction:column; gap:10px;">
                ...
                <p style="font-size:12px; font-weight:bold; color:var(--ink-blue); text-align:left; margin:0;">💬 Chat with AI (Gemini 2.0)</p>
